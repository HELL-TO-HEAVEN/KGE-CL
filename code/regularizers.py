...
class N3(Regularizer):
    ...
    def forward(self, factors):
        ...
        for factor in factors:
            h, r, t = factor
            norm += torch.sum(torch.abs(h) ** 3)
            norm += torch.sum(torch.abs(r) ** 3)
            norm += torch.sum(torch.abs(t) ** 3)
            norm += torch.sum(torch.abs(h*r*t))
            norm = self.weight * norm
        norm = norm / factors[0][0].shape[0]
        return norm
